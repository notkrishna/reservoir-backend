...
def delete_from_b2(file_url):
    ...
    file_path = f"post_imgs/{file_name}" # Extract the file name from the URL
    bucket = b2_api.get_bucket_by_name(settings.B2_BUCKET_NAME)
    ...
    try:
        ...
        bucket.delete_file_version(file_id, file_path)
        print("Sucessfully deleted obj from cloud")
    except FileNotPresent:
        print(f"file: {file_name} and {file_id} not present")
